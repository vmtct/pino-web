...
const passRules:Record<PassType,{accessScope:"Open Studio + Premium"|"Any"}>={Piano:{accessScope:"Open Studio + Premium"},Art:{accessScope:"Open Studio + Premium"},"Little Piner":{accessScope:"Open Studio + Premium"},"Bring-a-Friend":{accessScope:"Any"}};
...
async function issuePasses(env:Env,studentId:string,month:string,subscriptionId?:string){
  const monthDate=new Date(`${month}T00:00:00Z`); if(Number.isNaN(monthDate.getTime())) return {ok:false as const,status:400,error:"month must be YYYY-MM-01."};
  const nextMonth=new Date(Date.UTC(monthDate.getUTCFullYear(),monthDate.getUTCMonth()+1,1)).toISOString().slice(0,10);
  const existingResponse=await notionQuery(env,env.NOTION_OS_PASS_DATA_SOURCE_ID,{and:[{property:"Student",relation:{contains:studentId}},{property:"Month",date:{on_or_after:month}},{property:"Month",date:{before:nextMonth}}]});
  if(!existingResponse.ok){const detail=await existingResponse.text();return {ok:false as const,status:502,error:"Could not check existing passes.",notionStatus:existingResponse.status,detail:detail.slice(0,1000)}}
  const existing=await existingResponse.json() as {results?:Array<{id:string}>}; if((existing.results?.length||0)>0)return {ok:true as const,alreadyIssued:true,passCount:existing.results?.length||0,passIds:existing.results?.map(r=>r.id)||[]};
  const createdIds:string[]=[];
  for(const type of allowedPasses){const rule=passRules[type];const response=await notionCreatePage(env,{data_source_id:env.NOTION_OS_PASS_DATA_SOURCE_ID},{Name:{title:[{text:{content:`${type} · ${month}`}}]},Student:{relation:[{id:studentId}]},...(subscriptionId?{Subscription:{relation:[{id:subscriptionId}]}}:{}),"Pass Type":{select:{name:type}},Status:{select:{name:"Available"}},"Access Scope":{select:{name:rule.accessScope}},Month:{date:{start:month}}});if(!response.ok){const detail=await response.text();return {ok:false as const,status:502,error:"Could not issue passes.",notionStatus:response.status,detail:detail.slice(0,1000),createdIds}}const created=await response.json() as {id?:string};if(created.id)createdIds.push(created.id)}
...
async function getProgramContext(env:Env){
  const [classesResponse,programsResponse]=await Promise.all([
    ...
    notionQuery(env,env.NOTION_PATH_PROGRAM_DATA_SOURCE_ID),
  ]);
  if(!classesResponse.ok||!programsResponse.ok)return {classPaths:new Map<string,string>(),programPaths:new Map<string,string>()};
