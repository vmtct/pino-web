...
async function notionQuery(env:Env,dataSourceId:string,filter?:unknown){const response=await fetch(`https://api.notion.com/v1/data_sources/${dataSourceId}/query`,{method:"POST",headers:notionHeaders(env),body:JSON.stringify(filter?{filter}:{})});if(response.ok||dataSourceId!==env.NOTION_OS_SESSION_DATA_SOURCE_ID||!env.NOTION_OS_SESSION_DATABASE_ID)return response;return fetch(`https://api.notion.com/v1/databases/${env.NOTION_OS_SESSION_DATABASE_ID}/query`,{method:"POST",headers:notionHeaders(env,"2022-06-28"),body:JSON.stringify(filter?{filter}:{})});}
