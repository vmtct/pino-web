...
test('live staging: PIN login -> change PIN -> Open Studio reserve -> cancel', async ({ page }) => {
  ...
  try {
    ...
    createdClaimId = typeof admissionBody?.data?.claimId === 'string' ? admissionBody.data.claimId : null;
    ...
    const cancellationResponse = page.waitForResponse((response) =>
      response.url().includes(`/api/piner/students/${fixture.studentId}/open-studio/claims/`)
    ...
    if (createdClaimId && !cancellationConfirmed) {
      const cleanup = await page.context().request.post(
        `${STAGING_ORIGIN}/api/piner/students/${fixture.studentId}/open-studio/claims/${createdClaimId}/cancel`,
        {
      ...
      if (!cleanup.ok()) {
        throw new Error(`PINER_STAGING_CLEANUP_FAILED:${cleanup.status()}:${createdClaimId}`);
      }
