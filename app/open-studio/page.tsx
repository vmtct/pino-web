  ...
  return (
    <main className="intake-page open-studio-landing">
      <nav className="nav shell">
        ...
        <div className="nav-links"><a href="#sessions">Các buổi đang có</a><a href="#how">Cách hoạt động</a></div>
        <a className="nav-cta" href="#start">Khám phá Open Studio →</a>
      </nav>

      <section className="os-ad-hero shell">
        <div>
          <p className="eyebrow">PINO OPEN STUDIO</p>
          <h1>Nếu hôm nay con <em>được tự chọn?</em></h1>
          <p className="hero-lede">Một buổi chiều để con vẽ, chơi nhạc và khám phá điều mình thích. Không thêm áp lực. Chỉ là một khoảng thời gian thật sự của riêng con.</p>
          <div className="hero-actions"><a className="button button-dark" href="#sessions">Xem các buổi đang có →</a><a className="text-link" href="#start">Nhận Open Studio Pass</a></div>
          <div className="hero-proof"><span>ART</span><i>·</i><span>PIANO</span><i>·</i><span>LITTLE PINER</span><i>·</i><span>3–15 TUỔI</span></div>
        </div>
        <div className="hero-art os-hero-art" aria-hidden="true"><div className="orb orb-a" /><div className="orb orb-b" /><div className="orb orb-c" /><div className="scribble">✦</div><div className="art-note"><strong>make</strong><br />something today.</div></div>
      </section>
      ...
      <section className="os-sessions-section" id="sessions"><div className="shell">
        <div className="section-heading"><div><p className="eyebrow">THIS WEEK AT PINO</p><h2>Con có thể<br /><em>khám phá gì?</em></h2></div><p>Các session thực tế đang mở. Chỗ trống thay đổi theo từng buổi.</p></div>
        {sessionLoading ? <div className="session-loading">Đang xem lịch Open Studio…</div> : sessions.length ? <div className="session-grid">{sessions.slice(0, 6).map((session, index) => <article className={`session-card session-card-${(index % 3) + 1}`} key={session.id}><div className="session-card-top"><span>{session.type}</span><span>{session.path || "Open Studio"}</span></div><div className="session-card-body"><p className="session-date">{formatSessionDate(session.date)}</p><h3>{session.topic}</h3><p className="session-seats">{session.availableSeats === null ? "Đang cập nhật chỗ" : session.availableSeats === 0 ? "Đã đầy" : `${session.availableSeats} chỗ còn lại`}</p></div></article>)}</div> : <div className="session-empty">Lịch đang được cập nhật. Để lại thông tin bên dưới, PINO sẽ giúp gia đình chọn buổi phù hợp.</div>}
      </div></section>
      ...
      <section className="how-it-works" id="how"><div className="shell"><div className="section-heading"><div><p className="eyebrow">HOW IT WORKS</p><h2>Đơn giản<br /><em>thôi.</em></h2></div><p>Không cần biết trước con sẽ thích gì. Chỉ cần chọn một buổi để bắt đầu.</p></div><div className="steps-grid">
        <div className="step-card"><span>01 · CHỌN</span><h3>Chọn một buổi</h3><p>Xem lịch và tìm một session phù hợp với thời gian của gia đình.</p></div>
        <div className="step-card"><span>02 · KHÁM PHÁ</span><h3>Con đến PINO</h3><p>Một buổi Art, Piano hoặc Little Piner — với đủ khoảng trống để con tự khám phá.</p></div>
        <div className="step-card"><span>03 · TẠO RA</span><h3>Con làm một điều</h3><p>Một bức tranh, một giai điệu, một ý tưởng — thứ con mang về là của riêng con.</p></div>
        <div className="step-card"><span>04 · QUAY LẠI</span><h3>Thích thì khám phá tiếp</h3><p>Open Studio được thiết kế để mỗi lần quay lại có thể là một điều mới.</p></div>
      </div></div></section>
