...
type Session = {
  ...
  runningClassIds: string[];
  cover: string | null;
  avatar: string | null;
};

const filters = ["All", "Open Studio", "Premium"];
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f4f0e7", color: "#171713" }}>
      ...
      <section className="shell" style={{ padding: "95px 0 55px" }}>
        ...
        <p style={{ maxWidth: 570, color: "#777269", fontSize: 18, lineHeight: 1.6, marginTop: 28 }}>
          Sessions happen inside real PINO classes. Browse what is open, then use an eligible pass when you are ready to join.
        </p>
      </section>
      ...
      <section className="shell" style={{ paddingBottom: 100 }}>
        ...
        <div className="session-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 14 }}>
          {visible.map((session) => {
            ...
            return <article key={session.id} style={{ background: "#e8e0d1", border: "1px solid rgba(23,23,19,.12)", overflow: "hidden" }}>
              ...
              <div style={{ padding: 26 }}>
                <p style={{ margin: "0 0 10px", fontSize: 11, letterSpacing: ".12em", fontWeight: 700, color: "#777269" }}>{session.runningClassIds.length ? "RUNNING CLASS · OPEN FOR OS" : "PINO SESSION"}</p>
                <h2 style={{ fontSize: 32, lineHeight: 1, letterSpacing: "-.05em", margin: "0 0 10px" }}>{session.topic}</h2>
      ...
      <style jsx>{`@media(max-width:700px){.shell{width:calc(100% - 32px)!important}.session-grid{grid-template-columns:1fr!important}}`}</style>
    </main>
  );
}
